...
*/
contract L1Manager is UUPSUpgradeable, Initializable, OwnableUpgradeable {
    struct Request {
     address token;
     uint256 amount;
     address sender;    
    ...
    bytes32 constant STORAGE_SLOT = keccak256("L1_MANAGER_SETTINGS");
    ...
    // events list
    event InitMigration(uint256 id, address indexed token, uint256 amount, address indexed sender, uint256 indexed receiver);
    ...
    function _initialize(
        ...
        _storage().starknetCore = IStarknetCore(snMessaging);
    ...
    // override the UUPSUpgradeable function to add the authorization check
    ...
    function _fallback() internal {
        if (msg.value > 0) {
           payable(_storage().settings.fee_receiver).transfer(msg.value);
        }
    }
        ...
        payable
    {
        ...
        // collect fee
        uint256 _fee = _storage().settings.fee_eth;
        if (_fee > 0) {
            require(msg.value >= _fee, "Insufficient fee");
            uint256 balance = address(this).balance;
            require(balance >= _fee, "Insufficient balance");
            payable(address(_storage().settings.fee_receiver)).transfer(_fee);
            emit FeeReceived(msg.sender, _fee, _storage().settings.fee_receiver);
        }

        // increase request id
        uint256 current_id = ++_storage().current_request_id;
        ...
        if (tokenConfig.l1_token_address == address(0)) {
            ...
            // bridge eth
            IStarkgateTokenBridge(tokenConfig.bridge_address).deposit{ value: amount + deposit_fee }(amount, _storage().settings.l2_easyleap_receiver);
        } else {
            ...
            IStarkgateTokenBridge(tokenConfig.bridge_address).deposit{ value: deposit_fee }(amount, _storage().settings.l2_easyleap_receiver);
        }
        
        _sendMessage(_calldata);

        // - write to requests
        _storage().idToRequest[current_id] = Request({
        ...
        // emit InitMigration
        emit InitMigration(current_id, tokenConfig.l1_token_address, amount, msg.sender, _calldata[3]);
    }
    ...
    function _sendMessage(uint256[] memory _payload) internal {
        ...
        _storage().starknetCore.sendMessageToL2{
            value: bal
        }(_storage().settings.l2_easyleap_receiver, L2_SELECTOR, _payload);
    }

    function set_settings(Settings memory _settings) external onlyOwner {
        _storage().settings = _settings;
    }
    ...
    function _storage() internal pure returns (MyStorage storage store) {
